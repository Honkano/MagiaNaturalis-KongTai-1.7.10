...
public class ItemHerobrinesScythe extends ItemSword implements IRepairable, IWarpingGear {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List lst, boolean held) {
        lst.add(EnumChatFormatting.ITALIC + "这是自然魔法重生版迁移了神秘基础学");
        lst.add(EnumChatFormatting.ITALIC + "第一个象征意义的物品我是空太");
        lst.add(EnumChatFormatting.ITALIC + "一个用接手了自然魔法的普通人");
        // 【新增】自定义吸血标签，带动态吸血说明
        lst.add(EnumChatFormatting.RED + "");
        lst.add(EnumChatFormatting.RED + "");
        lst.add(EnumChatFormatting.RED + "吸血: 5% (血量越低，吸血越高)");
        lst.add(EnumChatFormatting.BLUE + "格挡: 100% 完全免疫伤害");
        lst.add(EnumChatFormatting.RED + "");
    }
            ...
            if (!p_71059_1_.hitByEntity(p)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag2) {
                        // ========================================================
                        // 【新增】动态吸血逻辑！
                        // 基础吸血 5%。计算玩家当前血量百分比，越低吸血越高。
                        // 满血时 5%，半血时约 12.5%，残血时最高 20%。
                        // ========================================================
                        float healthRatio = p.getHealth() / p.getMaxHealth();
                        float lifesteal = 0.05F + (1.0F - healthRatio) * 0.15F;
                        p.heal(f * lifesteal);
                        // ========================================================
    ...
    public Multimap getAttributeModifiers(ItemStack stack) {
        ...
        attribs.put(
            ...
            new AttributeModifier(UUID.fromString("CB3F55D3-645C-4F38-A497-9C13A33DB5CE"), "Speed modifier", 1.0F, 2));
